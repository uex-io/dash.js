...
describe('HTTPLoader', function () {
    ...
    it('should throw an exception when attempting to call load and config parameter has not been set properly', () => {
        ...
        expect(httpLoader.load.bind(httpLoader, { request: {}  ,mediaInfo: {streamInfo: {}}})).to.throw('config object is not correct or missing');
    });
    ...
    it('should use XHRLoader if it is not an arraybuffer request even if useFetch is set to true', () => {
        ...
        httpLoader.load({ request: {  mediaInfo: {streamInfo: {}}, checkExistenceOnly: true, responseType: 'json', type: HTTPRequest.MEDIA_SEGMENT_TYPE }, success: callbackSucceeded, complete: callbackCompleted, error: callbackError });
    ...
    it('should use XHRLoader and call success and complete callback when load is called successfully', () => {
        ...
        httpLoader.load({ request: { mediaInfo: {streamInfo: {}}, checkExistenceOnly: true }, success: callbackSucceeded, complete: callbackCompleted, error: callbackError });
        expect(self.requests.length).to.equal(1);
        self.requests[0].respond(200);
        sinon.assert.notCalled(global.fetch);
    ...
    it('should use XHRLoader if it is not a MEDIA_SEGMENT_TYPE request even if useFetch is set to true and it is an arraybuffer request', () => {
        ...
        httpLoader.load({ request: { mediaInfo: {streamInfo: {}}, checkExistenceOnly: true, responseType: 'arraybuffer', type: HTTPRequest.INIT_SEGMENT_TYPE}, success: callbackSucceeded, complete: callbackCompleted, error: callbackError });
        expect(self.requests.length).to.equal(1);
        self.requests[0].respond(200);
        sinon.assert.notCalled(global.fetch);
    });
    ...
    it('should use XHRLoader if it is an arraybuffer and  MEDIA_SEGMENT_TYPE request, useFetch is true and body is not an Stream. It should call success and complete callback when load is called successfully', (done) => {
        ...
        global.fetch.returns(Promise.resolve(new global.Response('', { status: 200 })));
        httpLoader.load({ request: { mediaInfo: {streamInfo: {}}, checkExistenceOnly: true, responseType: 'arraybuffer', type: HTTPRequest.MEDIA_SEGMENT_TYPE }, success: callbackSucceeded, complete: callbackCompleted, error: callbackError });
